...
.. TODO:: check/fill out procedure post refactoring

.. TODO:: better distinction between ``riscv-formal`` (the project) and ``riscv_formal`` (the CLI)
...
and a ``checks.cfg`` file and use the ``riscv_formal`` CLI to create
the formal checks. See ``cores/picorv32/`` for an
example implementation. The checks generated by ``riscv_formal`` are
...
- A configuration file with extension ``.cfg`` is used to configure the
  ``riscv_formal`` CLI.
- ``riscv_formal [options] <config>.cfg``

  + specifying ``.cfg`` is optional (i.e. ``riscv_formal checks`` will
    load config settings from a file named ``checks.cfg``)
  + can also give ``path/to/config``
  + will generate tests in directory called ``<config>``
...
.. note::
  ...
  `Verbatim sections`_ are not pre-parsed for comments.  In particular,
  sections which provide raw SystemVerilog code such as the `[defines]`_
  section may produce syntax errors if ``#`` comments are used, and
  should instead use SystemVerilog style comments with ``//``.
...
rv64i base instruction sets, e.g. ``isa rv32iZbc``.

.. TODO:: comment on extension name formatting (error message provided if invalid, so low priority)
...
The following checks are managed by ``riscv_formal`` and can be
...
.. TODO:: I don't think masks are available for the refactor yet
...
The following checks are not yet managed by ``riscv_formal`` and can not
be implemented using the standard RVFI wrapper interface. Some of them
may be integrated with ``riscv_formal`` in the future.
...
.. warning:: These checks are (currently) unmaintained and may not work

  as discussed in https://github.com/YosysHQ/riscv-formal/issues/30#issuecomment-2532886429
